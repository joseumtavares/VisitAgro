...
export default function SalesPage() {
  ...
  const [filterStatus, setFilter] = useState<'todos' | OrderStatus>('todos');
  ...
  const addItem = () =>
    setForm(f => ({
      ...
      items: [
        ...f.items,
        { product_id: '', product_name: '', quantity: 1, unit_price: 0, rep_commission_pct: 0 },
      ],
    }));
  ...
  const updateItem = (i: number, k: keyof OrderItem, v: string | number) =>
    setForm(f => {
      const items = [...f.items];
      items[i] = { ...items[i], [k]: v };
      if (k === 'product_id') {
        ...
        if (prod) {
          ...
          items[i].unit_price = Number(prod.unit_price) || 0;
          items[i].rep_commission_pct = Number(prod.rep_commission_pct) || 0;
        }
      }
      return { ...f, items };
    });
  ...
  const filtered = orders.filter(o => {
    ...
    return matchesSearch && (filterStatus === 'todos' || o.status === filterStatus);
  });
            ...
            return (
              <div key={s} className="bg-dark-800 rounded-xl border border-dark-700 p-4">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-dark-400 text-sm">{cfg.label}</span>
                  ...
                  {filtered.map(o => {
                    ...
                    return (
                      <tr key={o.id} className="hover:bg-dark-700/30 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-full border text-xs ${cfg.cls}`}
                          >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={e =>
                      setForm(f => ({
                        ...f,
                        referral_id: e.target.value || null,
                      }))
                    }
                    className="w-full bg-dark-900 border border-dark-700 rounded-lg px-3 py-2 text-white text-sm outline-none focus:ring-2 focus:ring-primary-500"
                  >
