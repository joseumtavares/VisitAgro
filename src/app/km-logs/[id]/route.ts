// src/app/api/km-logs/[id]/route.ts — Lote L040.1
//
// AI-CONTEXT: Segue padrão de src/app/api/orders/[id]/route.ts.
// CRITICAL: Buscar sempre com workspace + deleted_at IS NULL.
...
export async function PUT(
  ...
  // AI-RULE: representative só edita o próprio registro.
  ...
  const updatePayload: Record<string, unknown> = {
    km_ini: km_ini_new, km_fim: km_fim_new,
    percorrido, litros: litros_new, combustivel: comb_new, consumo, custo_por_km,
  };
  ...
  // DEPENDE DE: auditLog(workspace, userId, 'km_log_update', { id });

  return NextResponse.json({ km_log: updated });
}

// ---------------------------------------------------------------------------
// DELETE /api/km-logs/[id] — soft delete
// ---------------------------------------------------------------------------
export async function DELETE(
  ...
  // DEPENDE DE: auditLog(workspace, userId, 'km_log_delete', { id });
