// src/app/api/km-logs/route.ts — Lote L040.1
//
// AI-CONTEXT: Segue padrão de src/app/api/orders/route.ts (confirmado via L036-A).
// getRequestContext extrai workspace, userId, role do JWT injetado pelo middleware.
// getAdmin() usa service_role — RLS não aplica; filtros OBRIGATÓRIOS na aplicação.
//
...
// AI-RULE: representative vê apenas próprios registros.
//          admin/manager veem workspace completo e aceitam ?user_id=.
//          role 'user' recebe 403.
const ALLOWED_ROLES = ['admin', 'manager', 'representative'];

// ---------------------------------------------------------------------------
// GET /api/km-logs
// Query: from? (date), to? (date), user_id? (admin/manager only)
// ---------------------------------------------------------------------------
export async function GET(request: NextRequest) {
  ...
  // CRITICAL: workspace sempre filtrado.
  let query = getAdmin()
    .from('km_logs')
    .select('*')
  ...
  // AI-RULE: representative só vê os próprios registros.
  ...
  const kmLogs = data ?? [];
  const total_percorrido  = kmLogs.reduce((s: number, r: any) => s + (r.percorrido  ?? 0), 0);
  const total_litros      = kmLogs.reduce((s: number, r: any) => s + (r.litros      ?? 0), 0);
  const total_combustivel = kmLogs.reduce((s: number, r: any) => s + (r.combustivel ?? 0), 0);
...
export async function POST(request: NextRequest) {
  ...
  if (!dataStr || !veiculo || km_ini == null || km_fim == null) {
    return NextResponse.json(
      { error: 'Campos obrigatórios: data, veiculo, km_ini, km_fim.' },
      { status: 400 }
    );
  }

  // CRITICAL: backend é fonte de verdade para a validação km_fim >= km_ini.
  ...
  // Verificar duplicidade diária ativa
  ...
  // DEPENDE DE: auditLog — ativar após confirmar padrão real nas rotas irmãs
