 ...
 * - filtrar pedidos por cliente / número / status
 * - abrir modal para criar nova venda
 * - carregar entidades relacionadas (clientes, produtos, indicadores)
 * - para admin/manager, carregar também representantes
 * - garantir que a UI respeite a regra de negócio:
 *   - indicador é OPCIONAL
 *   - representante é OBRIGATÓRIO em toda venda
 *
 * Observação importante:
 * O vínculo do representante é salvo em orders.user_id.
 * Se esse campo for enviado vazio para admin/manager, a venda não deve ser salva.
 * Para usuário com role = representative, o backend deve vincular automaticamente
 * o pedido ao próprio usuário logado.
 * ──────────────────────────────────────────────────────────────────────────
 */
...
/**
 * Pedido listado na grade principal.
 *
 * Observação:
 * - user_id representa o representante responsável pela venda
 * - referral_id representa o indicador (opcional)
 */
interface Order {
  id: string;
...
/** Cliente usado no select do formulário */
interface Client {
  id: string;
  name: string;
}
...
/**
 * Produto usado no formulário.
 * rep_commission_pct é copiado para o item no momento da venda.
 */
interface Product {
  id: string;
  name: string;
...
/** Indicador / referral — opcional na venda */
interface Referral {
  id: string;
  name: string;
...
/** Representante listado para admin/manager */
 ...
 * Item da venda.
 *
 * rep_commission_pct é salvo no item para preservar snapshot da regra de comissão.
 */
interface OrderItem {
  product_id: string;
  product_name: string;
 ...
 * Mantém o mapeamento centralizado para evitar duplicação no JSX.
 */
...
export default function SalesPage() {
  ...
  /**
   * hydrated evita problemas de hydration mismatch em páginas client-side.
   * Só começamos a agir sobre autenticação depois que o componente monta.
   */
   ...
   * A UI usa isso para habilitar / ocultar partes do formulário.
   */
   ...
   *
   * Regras importantes:
   * - referral_id é opcional
   * - user_id é obrigatório para admin/manager
   * - para representative, o backend deve sobrescrever com o usuário logado
   */
  ...
  /**
   * Redireciona para login se a página já estiver hidratada e o usuário não estiver autenticado.
   */
  useEffect(() => {
    if (!hydrated) return;
   ...
   * - produtos
   * - indicadores
   * - representantes (somente admin/manager)
   */
  const load = useCallback(async () => {
    ...
    // Representative não precisa da lista porque o backend já vincula o pedido
    // automaticamente ao usuário logado.
    if (isAdminOrManager) {
      calls.push(apiFetch('/api/representatives').then(x => x.json()));
    }

    const results = await Promise.all(calls);
    const [o, c, p, r, repsResponse] = results;
    ...
    // Mantém apenas representantes ativos no select da venda.
    if (isAdminOrManager) {
      setRepresentatives(
        (repsResponse?.representatives ?? []).filter(
          (rep: Representative) => rep.active
        )
      );
    }
  ...
  const removeItem = (i: number) =>
    setForm((f: any) => ({
      ...f,
      items: f.items.filter((_: any, idx: number) => idx !== i),
    }));

  /**
   * Atualiza um item do formulário.
   *
   * Quando o product_id muda:
   * - copia nome do produto
   * - copia preço unitário
   * - copia rep_commission_pct para snapshot do item
   */
    ...
    setForm((f: any) => {
      ...
      if (k === 'product_id') {
        ...
        if (prod) {
          ...
          items[i].unit_price = prod.unit_price;
          items[i].rep_commission_pct = prod.rep_commission_pct;
        }
      }
  ...
  /** Total da venda calculado a partir dos itens */
   ...
   * Importante:
   * - indicador é opcional
   * - essa comissão NÃO substitui a comissão do representante
   */
  const calcCommission = () => {
    ...
    return ref.commission_type === 'percent'
   ...
   * - itens válidos são obrigatórios
   * - representante é obrigatório para admin/manager
   * - indicador é opcional e deve ser normalizado para null quando vazio
   */
  const save = async () => {
     ...
     * Representante é obrigatório em toda venda.
     *
     * - admin/manager: precisam selecionar explicitamente
     * - representative: backend usará o próprio usuário logado
     */
    if (isAdminOrManager && !form.user_id) {
      setError('Selecione o representante responsável pela venda.');
      return;
    }
    ...
    try {
       ...
       * referral_id é opcional.
       *
       * Se o usuário deixar “Sem indicador”, enviamos null para evitar
       * erro de FK em orders.referral_id.
       */
      const normalizedReferralId =
        form.referral_id && String(form.referral_id).trim() !== ''
          ? form.referral_id
          : null;
       ...
       *
       * - admin/manager: envia user_id selecionado
       * - representative: backend deve ignorar vazio e usar usuário logado
       */
      const payload = {
        ...form,
   ...
   *
   * O backend usa controle de versão otimista (version), então precisamos enviar
   * a versão atual para evitar overwrite concorrente.
   */
   ...
   *
   * Se a lista de representantes ainda não estiver carregada, exibimos fallback curto.
   */
  const repName = (userId: string | null | undefined) => {
    if (!userId) return '—';

    const rep = representatives.find(r => r.id === userId);
    if (rep) return rep.name || rep.username;
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-dark-400 text-sm mt-1">{orders.length} pedidos</p>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {(['pendente', 'aprovado', 'pago', 'cancelado'] as OrderStatus[]).map(s => {
            ...
            return (
              <div key={s} className="bg-dark-800 rounded-xl border border-dark-700 p-4">
        ...
        {/* Conteúdo principal: loading / vazio / tabela */}
        {loading ? (
                    ...
                    return (
                      <tr key={o.id} className="hover:bg-dark-700/30 transition-colors">
                        <td className="px-4 py-3">
                          <div className="text-white font-mono text-sm">#{o.order_number ?? '—'}</div>
                          <div className="text-dark-500 text-xs">{o.date ?? ''}</div>
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium border ${cfg.cls}`}
                          >
                            {cfg.label}
                          </span>
                        </td>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-dark-800 rounded-xl border border-dark-700 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              ...
              {/*
                Para representative, o backend vincula automaticamente orders.user_id.
                A UI só reforça esse comportamento com uma mensagem informativa.
              */}
              {isRepresentative && (
                <div className="bg-primary-600/10 border border-primary-500/30 text-primary-300 text-xs px-3 py-2 rounded-lg">
                  ...
                  Você não precisa selecionar representante.
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {/* Indicador é opcional por regra de negócio */}
                <div>
                  <label className="block text-xs text-dark-400 mb-1">Indicador</label>
                  <select
                    value={form.referral_id}
                    onChange={e => setForm((f: any) => ({ ...f, referral_id: e.target.value }))}
                    className="w-full bg-dark-900 border border-dark-700 rounded-lg px-3 py-2 text-white text-sm outline-none focus:ring-2 focus:ring-primary-500"
                  >
                ...
                {/* Seleção de representante: obrigatória para admin/manager */}
                {isAdminOrManager && (
                  <div>
                    <label className="block text-xs text-dark-400 mb-1">Representante *</label>
                    <select
                      value={form.user_id}
                      onChange={e => setForm((f: any) => ({ ...f, user_id: e.target.value }))}
                      className="w-full bg-dark-900 border border-dark-700 rounded-lg px-3 py-2 text-white text-sm outline-none focus:ring-2 focus:ring-primary-500"
                    >
                    ...
                    {/* Feedback visual para reforçar a regra de negócio */}
                    <p className="text-[11px] text-dark-500 mt-1">
                      Toda venda deve possuir um representante responsável.
                    </p>
                  </div>
                )}
                ...
                <div className="col-span-2">
                  ...
                  <textarea
                    value={form.obs}
                    onChange={e => setForm((f: any) => ({ ...f, obs: e.target.value }))}
                    rows={2}
              ...
              <div>
                ...
                {form.items.length === 0 && (
                  <p className="text-dark-600 text-sm text-center py-4">Nenhum produto adicionado</p>
                )}
                ...
                <div className="space-y-2">
                  {form.items.map((item: OrderItem, i: number) => (
                    <div
                      key={i}
                      className="bg-dark-900 rounded-lg p-3 grid grid-cols-12 gap-2 items-end"
                    >
                      <div className="col-span-5">
                        <label className="block text-xs text-dark-500 mb-1">Produto</label>
                        <select
                      ...
                      <div className="col-span-3">
                        <label className="block text-xs text-dark-500 mb-1">Preço Unit.</label>
                        <input
              ...
              {/* Totais e comissão de indicador */}
              {form.items.length > 0 && (
