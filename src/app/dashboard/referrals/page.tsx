'use client';
// L038: cards mobile adicionados; form corrigido para grid-cols-1 sm:grid-cols-2;
//       botões com min-h-[44px]; lógica 100% preservada.
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-dark-400 text-sm mt-1">{referrals.length} cadastrados</p>
          </div>
          {/* L038: min-h-[44px] + justify-center para mobile */}
          <button
            onClick={openNew}
            className="flex items-center justify-center gap-2 bg-primary-600 hover:bg-primary-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors min-h-[44px]"
          >
        ...
        {loading ? (
          ...
          <div className="text-center py-12 text-dark-400">Nenhum indicador encontrado</div>
        ) : (
          <>
            {/* L038: Cards mobile — visíveis apenas abaixo de sm */}
            <div className="sm:hidden space-y-3">
              {filtered.map(r => (
                <div key={r.id} className="bg-dark-800 rounded-xl border border-dark-700 p-4 space-y-3">
                  {/* Nome + comissão */}
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <div className="text-white font-medium text-sm">{r.name}</div>
                      {r.document && <div className="text-dark-500 text-xs mt-0.5">CPF: {r.document}</div>}
                    </div>
                    <span className={`inline-flex shrink-0 items-center gap-1 px-2 py-1 rounded-full text-xs font-medium border ${r.commission_type==='percent'?'bg-blue-500/20 text-blue-400 border-blue-500/30':'bg-green-500/20 text-green-400 border-green-500/30'}`}>
                      {r.commission_type==='percent'
                        ? <><Percent className="w-3 h-3"/>{r.commission_pct}%</>
                        : <><DollarSign className="w-3 h-3"/>R$ {Number(r.commission??0).toFixed(2)}</>}
                    </span>
                  </div>
                    ...
                    {filtered.map(r=>(
                      <tr key={r.id} className="hover:bg-dark-700/30 transition-colors">
                        <td className="px-4 py-3">
                          <div className="text-white font-medium text-sm">{r.name}</div>
                          {r.document&&<div className="text-dark-500 text-xs">CPF: {r.document}</div>}
                        </td>
                        <td className="px-4 py-3">
                          <div className="space-y-0.5 text-xs text-dark-300">
                            {r.tel&&<div>📞 {r.tel}</div>}
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-dark-800 rounded-xl border border-dark-700 w-full max-w-xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              ...
              {/* L038: grid-cols-1 sm:grid-cols-2 — corrige grid-cols-2 sem breakpoint */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="sm:col-span-2">
                  <label className="block text-xs text-dark-400 mb-1">Nome *</label>
                  <input value={form.name??''} onChange={e=>f('name',e.target.value)} className="w-full bg-dark-900 border border-dark-700 rounded-lg px-3 py-2 text-white text-sm outline-none focus:ring-2 focus:ring-primary-500"/>
                </div>
                <div>
                  <label className="block text-xs text-dark-400 mb-1">CPF</label>
                  <input value={form.document??''} onChange={e=>f('document',e.target.value)} placeholder="000.000.000-00" className="w-full bg-dark-900 border border-dark-700 rounded-lg px-3 py-2 text-white text-sm outline-none focus:ring-2 focus:ring-primary-500"/>
                </div>
                <div>
                  <label className="block text-xs text-dark-400 mb-1">Telefone</label>
                  <input value={form.tel??''} onChange={e=>f('tel',e.target.value)} placeholder="(48) 99999-9999" className="w-full bg-dark-900 border border-dark-700 rounded-lg px-3 py-2 text-white text-sm outline-none focus:ring-2 focus:ring-primary-500"/>
                </div>
                <div className="sm:col-span-2">
                  <label className="block text-xs text-dark-400 mb-1">Email</label>
                  <input value={form.email??''} onChange={e=>f('email',e.target.value)} type="email" className="w-full bg-dark-900 border border-dark-700 rounded-lg px-3 py-2 text-white text-sm outline-none focus:ring-2 focus:ring-primary-500"/>
                </div>

                <div>
                  <label className="block text-xs text-dark-400 mb-1">Tipo de Comissão</label>
                ...
                {form.commission_type==='percent'
                  ?<div>
                    <label className="block text-xs text-dark-400 mb-1">Percentual (%)</label>
                    <input value={form.commission_pct??0} onChange={e=>f('commission_pct',parseFloat(e.target.value)||0)} type="number" step="0.1" min="0" max="100" className="w-full bg-dark-900 border border-dark-700 rounded-lg px-3 py-2 text-white text-sm outline-none focus:ring-2 focus:ring-primary-500"/>
                  </div>
                  :<div>
                    <label className="block text-xs text-dark-400 mb-1">Valor Fixo (R$)</label>
                    <input value={form.commission??0} onChange={e=>f('commission',parseFloat(e.target.value)||0)} type="number" step="0.01" min="0" className="w-full bg-dark-900 border border-dark-700 rounded-lg px-3 py-2 text-white text-sm outline-none focus:ring-2 focus:ring-primary-500"/>
                  </div>}

                <div className="sm:col-span-2 pt-2 border-t border-dark-700">
                  <p className="text-xs text-dark-400 mb-3 font-medium uppercase tracking-wide">Dados Bancários</p>
                  {/* L038: grid bancário também corrigido para mobile */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label className="block text-xs text-dark-400 mb-1">Banco</label>
                      <input value={form.bank_name??''} onChange={e=>f('bank_name',e.target.value)} placeholder="Ex: Nubank, Bradesco" className="w-full bg-dark-900 border border-dark-700 rounded-lg px-3 py-2 text-white text-sm outline-none focus:ring-2 focus:ring-primary-500"/>
                    </div>
                    <div>
                      <label className="block text-xs text-dark-400 mb-1">Agência</label>
                      <input value={form.bank_agency??''} onChange={e=>f('bank_agency',e.target.value)} className="w-full bg-dark-900 border border-dark-700 rounded-lg px-3 py-2 text-white text-sm outline-none focus:ring-2 focus:ring-primary-500"/>
                    </div>
                    <div>
                      <label className="block text-xs text-dark-400 mb-1">Conta</label>
                      <input value={form.bank_account??''} onChange={e=>f('bank_account',e.target.value)} className="w-full bg-dark-900 border border-dark-700 rounded-lg px-3 py-2 text-white text-sm outline-none focus:ring-2 focus:ring-primary-500"/>
                    </div>
                    <div>
                      <label className="block text-xs text-dark-400 mb-1">Chave Pix</label>
                      <input value={form.bank_pix??''} onChange={e=>f('bank_pix',e.target.value)} placeholder="CPF, email, telefone..." className="w-full bg-dark-900 border border-dark-700 rounded-lg px-3 py-2 text-white text-sm outline-none focus:ring-2 focus:ring-primary-500"/>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            {/* L038: footer com min-h-[44px] e empilhamento no mobile */}
            <div className="p-6 border-t border-dark-700 flex flex-col sm:flex-row gap-3 justify-end">
              <button onClick={()=>setShowModal(false)} className="px-4 py-2 text-dark-400 hover:text-white text-sm min-h-[44px]">Cancelar</button>
              <button onClick={save} disabled={saving} className="px-6 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-lg text-sm font-medium disabled:opacity-50 min-h-[44px]">
