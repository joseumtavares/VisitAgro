'use client';
/**
 * src/app/dashboard/sales/page.tsx — Lote L036-A
 *
 * MUDANÇA: controle de acesso por perfil no frontend.
 *   - representative: vê apenas as próprias vendas (filtro já feito na API);
 *     não pode alterar status de pedidos alheios.
 *   - admin/manager: comportamento anterior completo.
 *
 * O botão "Nova Venda" permanece disponível para todos os perfis
 * (representative cria pedidos vinculados ao próprio user_id automaticamente).
 *
 * Alterações cirúrgicas:
 *   1. Lê user?.role do authStore para saber se é representative.
 *   2. Esconde seletor de status para representative (segurança UX).
 *   3. Nenhuma alteração na lógica de POST/PUT — o controle real está na API.
 */
...
export default function SalesPage() {
  ...
  // ── Perfil: representative não pode alterar status ────────────────────────
  const isRepresentative = user?.role === 'representative';
  const canChangeStatus  = !isRepresentative; // admin, manager, user podem
  ...
  const save = async () => {
    ...
    try {
      ...
      await load();
      setShowModal(false);
      setForm({ client_id: '', referral_id: '', status: 'pendente', date: new Date().toISOString().split('T')[0], obs: '', items: [] });
      setError('');
    } catch (e: any) {
      setError(e.message);
    } finally {
  ...
  const changeStatus = async (id: string, status: OrderStatus) => {
    ...
    if (!order || typeof order.version !== 'number') { setError('Não foi possível identificar a versão atual do pedido.'); return; }
    ...
    <DashboardShell>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <select
            value={filterStatus} onChange={e => setFilter(e.target.value as any)}
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-dark-700 text-dark-400 text-xs uppercase">
                    ...
                    {/* Coluna Ação: apenas para quem pode alterar status */}
                  ...
                  {filtered.map(o => {
                    ...
                    return (
                      <tr key={o.id} className="hover:bg-dark-700/30 transition-colors">
                        ...
                        {/* Seletor de status: apenas para admin/manager/user */}
                        {canChangeStatus && (
      ...
      {/* Modal Nova Venda — disponível para todos os perfis */}
      {showModal && (
                  ...
                  {form.items.map((item: OrderItem, i: number) => (
                    <div key={i} className="bg-dark-900 rounded-lg p-3 grid grid-cols-12 gap-2 items-end">
                      ...
                      <div className="col-span-2 flex items-end justify-end pb-1">
                        ...
                        <button onClick={() => removeItem(i)} className="ml-2 text-dark-500 hover:text-red-400">
