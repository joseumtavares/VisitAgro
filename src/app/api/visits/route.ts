...
// ARQUIVO NOVO — precisa ser commitado no GitHub para existir no deploy da Vercel
// Causa do erro "<!DOCTYPE": este arquivo não existia no repositório, Vercel retornava 404 HTML
...
export async function GET(req: NextRequest) {
  ...
  try {
    let q = getAdmin()
      .from('visits')
      .select('id,client_id,user_id,activity_type,scheduled_date,visit_date,status,obs,lat,lng,created_at,updated_at')
    ...
    console.error('[visits GET]', e.message);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const payload: Record<string, any> = {
      ...
      created_at: now,
      updated_at: now,
      lat: (body.lat && body.lat !== 0) ? body.lat : 0,
      lng: (body.lng && body.lng !== 0) ? body.lng : 0,
    };
    ...
    // Atualiza status do cliente se checkin
    ...
    // Agenda próxima visita se solicitado
    ...
    await auditLog(
      isCheckin ? '[CHECKIN] Visita registrada' : '[VISITA] Agendamento criado',
      {
        ...
        client_status: body.client_status ?? null,
      },
      userId
    );
